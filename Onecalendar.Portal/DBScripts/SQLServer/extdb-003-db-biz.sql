...
BEGIN
	...
	ORDER BY t.CREATED_TIME,t.COURSE_NAME DESC
...
IF OBJECT_ID( 'dbo.P_QUERY_COURSE_BY_PARAMETER', 'P' ) IS NOT NULL
  DROP  PROCEDURE  dbo.P_QUERY_COURSE_BY_PARAMETER
GO

CREATE PROCEDURE [dbo].[P_QUERY_COURSE_BY_PARAMETER]
(
  @p_category nvarchar(36),
  @p_location nvarchar(36),
  @p_orderby nvarchar(36)
)
AS
/*
Module  : 
...
BEGIN
    ...
    --DECLARE @p_category varchar(64)='test',
	--@p_location varchar(64)='test',  
	--@p_orderby varchar(64)='date'; 
SELECT 
      t.COURSEID,
      t.USERID,
      t.COURSE_NAME,
	...
	WHERE t.STATUS = 'A' 
	AND t.USERID is not null-- only approved will be search out.
	AND (@p_category !='NULL' AND (charindex(@p_category, t.COURSE_NAME)>0 OR charindex(@p_category, t.COURSE_DETAIL)>0))
	AND (@p_location !='NULL' AND charindex(@p_location, t.COURSE_DETAIL)>0)
	ORDER BY
		CASE WHEN @p_orderby = 'date' then t.CREATED_TIME 
		END DESC,
		CASE WHEN @p_orderby = 'title' then t.COURSE_NAME
		END DESC
